...
function calculateCoCastStats(results, casts) {
    ...
    let html = `<div style="margin-bottom:12px; font-weight:600; font-size:1.1rem; color:var(--primary-color);">${casts.join(' & ')} 已经同台了 <span style="font-size:1.4rem; margin:0 4px;">${total}</span> 场</div><div style="display:flex; flex-direction:column; gap:8px;">`;
    Object.keys(groupMap).sort((a, b) => groupMap[b].total - groupMap[a].total).forEach(title => {
        const group = groupMap[title];
        html += `<div style="display:flex; flex-direction:column; padding:8px 12px; background:rgba(99, 126, 96, 0.04); border-radius:10px;"><div style="font-weight:600; color:var(--text-primary); margin-bottom:4px;">《${title}》 <span style="color:var(--primary-color); margin-left:8px;">${group.total}场</span></div>`;
        Object.keys(group.roles).sort((a, b) => group.roles[b] - group.roles[a]).forEach(role => {
            html += `<div style="font-size:0.85rem; color:var(--text-secondary); padding-left:12px; margin-top:2px;">${role} <span style="opacity:0.8; margin-left:10px;">${group.roles[role]}场</span></div>`;
        });
        html += `</div>`;
    });
    html += `</div>`;
